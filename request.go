...
func (rq *Request) releaseBuffersLocked() (buffers *requestBuffers) {
	...
	// rq.mu/muQueue.
	buffers = rq.buffers
	clear(rq.todoDirt) // release tag and targeted Element references before pooling
...
func (rq *Request) runWebSocket(ws *websocket.Conn, idleInterval, pingTimeout time.Duration) (err error) {
	...
	if err = rq.onConnect(); err == nil {
		...
		go wire.ReadLoop(ctx, cancelRequest, rq.Jaws.Done(), incomingMsgCh, idleInterval, pingTimeout, ws) // closes incomingMsgCh
...
func (rq *Request) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if rq.startServe() {
		...
		pingTimeout := rq.Jaws.getWebSocketTimeout()
		...
		if err == nil {
			...
			if err = rq.runWebSocket(ws, idleInterval, pingTimeout); err != nil {
