...
func tagExpand(l int, ctx Context, tag any, result []any) ([]any, error) {
	...
	case TagGetter:
		...
		return tagExpand(l+1, ctx, newTag, result)
	case []any:
		var err error
		for _, v := range data {
			if result, err = tagExpand(l+1, ctx, v, result); err != nil {
				break
			}
		}
		return result, err
...
func TagExpand(ctx Context, tag any) ([]any, error) {
	return tagExpand(0, ctx, tag, nil)
}
