...
func (e errNotUsableAsTag) Error() (s string) {
	...
	if e.tagGetterType != nil {
		return s + fmt.Sprintf("; found nested TagGetter at %s (%s); hint: implement JawsGetTag(*Request) on this type to delegate to that value, or pass that nested TagGetter directly", e.tagGetterPath, e.tagGetterType)
	}
	return s + "; found no nested TagGetter; hint: use a comparable tag value, or implement JawsGetTag(*Request) and return a comparable tag"
}
