...
import (
	...
	tagpkg "github.com/linkdata/jaws/core/jawstags"
)
...
func testBind_Hook_Success[T comparable](t *testing.T, testval T) {
	...
	tags1 := tagpkg.MustTagExpand(nil, bind1)
	...
	tags2 := tagpkg.MustTagExpand(nil, bind2)
...
func TestBind_Hook_Clicked_binding(t *testing.T) {
	...
	tags := tagpkg.MustTagExpand(nil, bind)
...
func TestBind_Hook_Clicked_bindingHook(t *testing.T) {
	...
	tags := tagpkg.MustTagExpand(nil, clickBind2)
	if !reflect.DeepEqual(tags, []any{&val}) {
		t.Error(tags)
	}
}
...
func TestBindFormat(t *testing.T) {
	...
	tags := tagpkg.MustTagExpand(nil, getter)
	if !reflect.DeepEqual(tags, []any{&val}) {
		t.Error(tags)
	}
