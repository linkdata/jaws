...
| Oversized payload (>32 KiB) | Rejected: inbound messages are capped at 32 KiB (`webSocketReadLimit`, set via `ws.SetReadLimit` in `request.go`); a larger message fails the read, is reported through the configured logger, and closes the connection |
