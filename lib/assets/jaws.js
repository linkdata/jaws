...
var jaws = null;
var jawsFailureGraceTimer = null;
var jawsIdPrefix = 'Jid.';
var jawsDebug = false;
const jawsJidRx = /^[1-9]\d*$/;
const jawsMaxJid = '9223372036854775807';
// Milliseconds after WebSocket failure before the lost indicator and probes begin.
...
function jawsLost() {
	...
	jawsShowLost();
	const elapsed = Math.floor((Date.now() - jaws) / 1000);
	const delay = Math.max(1, Math.min(60, elapsed));
...
function jawsFailed() {
	if (jaws instanceof WebSocket) {
		jaws = new Date();
		jawsFailureGraceTimer = setTimeout(function() {
			jawsFailureGraceTimer = null;
			jawsShowLost();
			jawsReconnect();
		}, jawsFailureGracePeriod);
	}
}

function jawsUnloading() {
	if (jawsFailureGraceTimer !== null) {
		clearTimeout(jawsFailureGraceTimer);
		jawsFailureGraceTimer = null;
	}
	if (jaws instanceof WebSocket) {
