...
const jawsNumberBaselines = new WeakMap();
...
function jawsSendNumber(elem) {
	if (!jawsCanSend() || !jawsIsJid(elem.id) || !jawsNumberBaselines.has(elem)) {
		return;
	}
	const val = elem.value;
	if (jawsNumberBaselines.get(elem) !== val) {
		jaws.send("Input\t" + elem.id + "\t" + JSON.stringify(val) + "\n");
		jawsNumberBaselines.set(elem, val);
	}
}

function jawsNumberChangeHandler(e) {
	if (e instanceof Event) {
		// Preserve the order when a different Number remains focused. If the
		// event belongs to that Number, jawsSendNumber deduplicates the second call.
		jawsFlushNumber();
		jawsSendNumber(e.currentTarget);
	}
}

function jawsFlushNumber() {
	const elem = document.activeElement;
	if (elem != null && jawsNumberBaselines.has(elem)) {
		jawsSendNumber(elem);
	}
}
...
function jawsAttach(elem) {
	...
	if (jawsIsInputTag(elem.tagName)) {
		if (elem.hasAttribute("data-jawsnumber")) {
			jawsNumberBaselines.set(elem, elem.value);
			elem.addEventListener('change', jawsNumberChangeHandler, false);
		} else {
...
function jawsPerform(what, id, data) {
	...
	switch (what) {
		...
		case 'Value':
			...
			if (jawsNumberBaselines.has(elem)) {
				jawsNumberBaselines.set(elem, elem.value);
			}
			return;
		case 'Append':
