...
function jawsAttach(elem) {
	...
	if (elem.hasAttribute("data-jawsname")) {
		...
		if (!Object.hasOwn(window.jawsNames, name) && name in window) {
			jawsCollidingVars[name] = undefined;
		}
...
function jawsVar(name, data, operation) {
	...
	if (keys.length > 0) {
		name = keys[0];
		let obj = window;
		if (operation !== 'Call' && Object.hasOwn(jawsCollidingVars, name)) {
			obj = jawsCollidingVars;
		}
...
const jawsCollidingVars = Object.create(null);
window.jawsNames = {};
