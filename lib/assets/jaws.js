...
const jawsNumberValues = new WeakMap();
...
function jawsSendNumber(elem) {
	if (!jawsCanSend() || !jawsIsJid(elem.id) || !jawsNumberValues.has(elem)) {
		return;
	}
	const val = elem.value;
	if (jawsNumberValues.get(elem) !== val) {
		jaws.send("Input\t" + elem.id + "\t" + JSON.stringify(val) + "\n");
		jawsNumberValues.set(elem, val);
	}
}
...
function jawsFlushNumber() {
	...
	if (elem != null && jawsNumberValues.has(elem)) {
		jawsSendNumber(elem);
	}
}
...
function jawsAttach(elem) {
	...
	if (jawsIsInputTag(elem.tagName)) {
		if (elem.hasAttribute("data-jawsnumber")) {
			jawsNumberValues.set(elem, elem.value);
