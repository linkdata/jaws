...
// ContentSecurityPolicy returns the policy generated by [Jaws.GenerateHeadHTML].
...
// Content-Security-Policy value with [Jaws.ContentSecurityPolicy] for each request.
// The policy allows resources configured by [Jaws.GenerateHeadHTML] when their
// URLs are supported by [secureheaders.BuildContentSecurityPolicy].
//
...
// [assets.PreloadHTML], along with the JaWS JavaScript and stylesheet. It also
// generates a Content-Security-Policy for URLs supported by
// [secureheaders.BuildContentSecurityPolicy], available from
// [Jaws.ContentSecurityPolicy]. If an extra resource's base name begins with
// "favicon" and its MIME type is image/*, its URL is stored and can be retrieved
// using [Jaws.FaviconURL].
//
